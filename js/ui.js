...
class UI {
  ...
  _showTooltip(e, invIndex) {
    ...
    this.els.tooltipName.textContent = `${def.icon || '📦'} ${item.type}`;
  ...
  updateCraftingPanel(player, crafting, workstationType = null) {
    ...
    const title = workstationType
      ? `🔨 ${workstationType}`
      : '🔨 Forge de base';

    if (this.els.craftTitle) this.els.craftTitle.textContent = title;
    if (this.els.craftWsLabel) {
      this.els.craftWsLabel.textContent = workstationType
        ? `Utilisation: ${workstationType}`
        : 'Forge de base — approchez un établi pour débloquer plus de recettes';
    }

    const recipes = crafting.getAvailableRecipes(player.inventory, workstationType);

    if (this.els.recipeList) {
      ...
      for (const r of recipes) {
        ...
        card.className = 'recipe-card' + (r.canCraft ? ' can-craft' : ' cannot-craft');

        const icon = crafting.getRecipeIcon(r);
        const catLabel = crafting.getCategoryLabel(r.category);

        let ingredientsHtml = r.ingredientStatus.map(ing =>
          `<div class="recipe-ingredient ${ing.enough ? 'has-enough' : 'not-enough'}">
            ${window.ITEM_DB?.[ing.type]?.icon || '📦'} ${ing.type}: ${ing.have}/${ing.count}
          </div>`
        ).join('');

        card.innerHTML = `
          <div class="recipe-name">${icon} ${r.name} <span style="font-size:0.65rem;opacity:0.5;">${catLabel}</span></div>
          <div class="recipe-ingredients">${ingredientsHtml}</div>
          <div class="recipe-result">→ ${icon} ${r.result.type} × ${r.result.count}${r.time > 0 ? ` (${r.time}s)` : ''}</div>
          <button class="recipe-craft-btn" ${r.canCraft ? '' : 'disabled'} data-recipe-id="${r.id}">
            ${r.canCraft ? (r.time > 0 ? `⚗ Fondre (${r.time}s)` : '✓ Fabriquer') : '✗ Ressources manquantes'}
          </button>
        `;
        ...
        if (btn && r.canCraft) {
          btn.addEventListener('click', () => {
            ...
            if (result.success) {
              if (result.item) this.showPickup(`${icon} ${result.item.type} × ${result.item.count}`);
              // Refresh panel
              this.updateInventoryGrid(player);
              this.updateCraftingPanel(player, crafting, workstationType);
