...
class World {
  ...
  getHeightAt(x, z) {
    ...
    let mask = 1.0;
    if (dist > WORLD.ISLAND_RADIUS - 20) {
      mask = Math.max(0, 1 - (dist - (WORLD.ISLAND_RADIUS - 20)) / 20);
    }
    
    const baseH = this.noise1(x * 0.02, z * 0.02) * 8 + 4;
    const detail = this.noise2(x * 0.08, z * 0.08) * 2;
    const peak = Math.max(0, 20 - dist * 0.5);
    
    let h = (baseH + detail + peak) * mask;
    if (h < -2) h = -2;
    return h;
  }
